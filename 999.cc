getgenv().Evolution = {
    ...
    ["Panic"] = {
        ...
        ["Things To Disable"] = {
            ["Options"] = false, -- // Stats, Notifications
            ["Aiming"] = false, -- // Silent Aim, Cam Lock
            ["FOV"] = false, -- // Silent Aim FOV, Cam Lock FOV
            ["Misc"] = false, -- // Macro, Gun Sorting, Fake Spike, Chat Commands
            ["Emotes"] = false, -- // Emotes
        }
    },
    ["Extra"] = {
...
if readfile(utility.Folder.."/invite.txt") ~= utility.Invite then 
    if httprequest then
        httprequest({
            Url = 'https://discord.gg/FbNm8WDdZa',
            Method = 'POST',
            Headers = {
...
if Evolution["Options"]["Internal"].Enabled then 
    if Evolution_UI_LOADED then 
        ...
        local Misc = Tabs.Extra:AddLeftGroupbox('Macro')
